...
6.1.3.7
	...
	- Example: Data được thiết kế phân stream một cách cụ thể 
...
7.1.1.2 File systems: definitions and conventions
	- Một trong những vấn đề phổ biến nhất trong công việc của programmer là xử lý dữ liệu được lưu trữ trong file. Hãy tưởng tượng một chương trình sắp xếp 20 số và người dùng chương trình này nhập 20 số này trực tiếp từ bàn phím. Bây giờ hãy tưởng tượng nhiệm vụ tương tự khi có 20.000 số cần sắp xếp – không có người dùng nào có thể nhập những số này mà không mắc lỗi. Sẽ dễ hình dung hơn nhiều khi những con số này được lưu trữ trong file đĩa được chương trình của bạn đọc.

	- Nếu chúng ta muốn triển khai ngay cả những cơ sở dữ liệu đơn giản nhất, cách duy nhất để lưu trữ thông tin giữa các lần chạy chương trình là lưu nó vào một file (hoặc các file, nếu cơ sở dữ liệu của bạn phức tạp hơn). Về nguyên tắc, hầu hết mọi vấn đề lập trình đều dựa vào việc sử dụng file, cho dù nó xử lý hình ảnh (được lưu trữ trong file), nhân ma trận (được lưu trữ trong file) hay tính toán tiền lương và thuế (đọc dữ liệu được lưu trữ trong file).
...
7.1.1.4
	- Nếu ta quan tâm đến 1 file cụ thể trong dir directory, tên là file. Giả sử gán 1 chuỗi chứa tên file:
...
7.1.2.1 Introduction to files and streams
	...
	- Programmer, có một bộ các hàm được định nghĩa chủ yếu trong file đầu vào stdio.h, có thể thực hiện một số thao tác trên stream ảnh hưởng đến các tập tin thực sự bằng cách sử dụng các cơ chế chứa trong operating system kernel. 

	- Như vậy, có thể truy cập vào bất kỳ file nào, ngay cả khi tên của file không được biết vào thời điểm viết chương trình.

	- Các thao tác được thực hiện với stream trừu tượng phản ánh các hoạt động liên quan đến file vật lý. Để link stream với file, bạn cần thực hiện một thao tác rõ ràng.

	- Thao tác link stream với một file được gọi là opening the file, trong khi disconnecting link này được gọi là closing the file. 

	=> Có thể kết luận rằng thao tác đầu tiên được thực hiện trên stream luôn luôn là opening và thao tác cuối cùng là closing. 

	- Chương trình thực tế có thể thao tác trên stream giữa hai sự kiện này và xử lý file tương ứng. Tuy nhiên, sự tự do này bị hạn chế bởi các đặc điểm vật lý của file và cách file đã được mở.


	- Tuy nhiên, việc mở stream có thể thất bại vì một số lý do: 
		. Lý do phổ biến nhất là lack of a file with a specified name. 
		. Hoặc là file vật lý tồn tại, nhưng chương trình không được phép mở nó. 
		. Hoặc là chương trình đã mở quá nhiều streams và hệ điều hành cụ thể có thể không cho phép mở đồng thời hơn N (ví dụ: 20) file. Một well-written program nên phát hiện các failed opening này và react accordingly.

	- Chúng ta có thể làm gì với một stream?
		. Việc mở stream liên quan đến file và cũng nên khai báo cách mà stream sẽ được xử lý. 
	...
	- Có hai thao tác cơ bản được thực hiện trên stream:
		...
		. read from the stream: các phần dữ liệu được "retrieved" từ file và đặt vào một khu vực bộ nhớ được quản lý bởi chương trình (ví dụ: một biến);
...
7.1.2.2
	...
	- Làm thế nào chúng ta biểu diễn một luồng trong chương trình?
		...
		. Tiêu chuẩn ngôn ngữ "C" giả định rằng môi trường lập trình (bao gồm compiler, các file đầu vào và thư viện liên quan, cũng như một số công cụ bổ sung) nên đảm bảo sự tồn tại của một kiểu có tên là FILE, được sử dụng để biểu diễn các luồng trong chương trình.
		...
		. Một biến có kiểu FILE được tạo ra khi bạn mở file và bị hủy khi file được đóng. Trong khoảng thời gian giữa hai sự kiện này, bạn có thể sử dụng dữ liệu này để chỉ định những thao tác nào nên được thực hiện trên một luồng cụ thể.
...
7.1.2.3
	- Binary streams vs text streams
		. Do loại nội dung của stream, all các stream được chia thành stream văn bản và stream nhị phân.

		. Các stream văn bản được structured in line; tức là chúng chứa các ký tự chữ (chữ cái, chữ số, dấu câu, v.v.) được sắp xếp thành các hàng (dòng), có thể thấy khi xem nội dung của một file trong trình soạn thảo. file này được viết (hoặc đọc) chủ yếu theo từng ký tự hoặc từng dòng.

		. Các stream nhị phân không chứa text mà là một "chuỗi các byte có giá trị bất kỳ". Chuỗi này có thể là một chương trình thực thi, một hình ảnh, một đoạn âm thanh hoặc video, một file cơ sở dữ liệu,... . Vì những file này không chứa các dòng, việc đọc và viết liên quan đến các phần dữ liệu có kích thước bất kỳ. Do đó, dữ liệu được đọc/viết theo từng byte hoặc từng khối, trong đó kích thước của một khối thường dao động từ 1 đến một giá trị được chọn một cách tùy ý.
		...
		. Sự mơ hồ này có thể gây ra nhiều hậu quả không mong muốn. Nếu bạn tạo một chương trình có trách nhiệm xử lý một file văn bản và viết nó cho hệ thống Windows, bạn có thể nhận diện kết thúc của các dòng bằng cách tìm các ký tự \r\n, nhưng chương trình tương tự được biên dịch và chạy trong môi trường Unix sẽ hoàn toàn không hữu ích và ngược lại. Những đặc điểm không mong muốn của một chương trình, gây ra việc ngăn chặn hoặc gây trở ngại trong việc sử dụng chương trình trong các môi trường khác nhau, được gọi là non-portability. Tương tự, đặc điểm của chương trình cho phép biên dịch và thực thi trong các môi trường khác nhau được gọi là portability. Một chương trình có tính di động được gọi là portable program.


7.1.2.4
	- Giải quyết vấn đề của portability tại library functions level - thứ chịu trách nhiệm cho việc đọc và ghi ký tự từ stream và đến stream. Quá trình diễn ra như sau:

		. Khi stream được open, dữ liệu trong file tương ứng nên được xử lý dưới dạng text;

		. Trong quá trình đọc/ghi dòng từ/vào file tương ứng, không có gì đặc biệt xảy ra trong môi trường Unix, nhưng khi thực hiện các hoạt động tương tự trong môi trường Windows, một quá trình gọi là dịch các ký tự xuống dòng diễn ra: khi bạn đọc một dòng từ file, mỗi cặp ký tự \r\n sẽ được thay thế bằng một ký tự \n duy nhất. Và ngược lại, trong quá trình ghi, mỗi ký tự \n sẽ được thay thế bằng một cặp ký tự \r\n;
		
		. Cơ chế này giúp những chương trình được viết chỉ để xử lý file văn bản Unix; thì source code cũng có thể được biên dịch trong môi trường Windows, và cũng sẽ hoạt động đúng.
...
7.1.3.1 Opening streams
	- The fopen() fucntion:
		...
		. Tham số đầu tiên của hàm chỉ định tên của file mà muốn gắn với stream. Tên file phải được viết theo các quy ước áp dụng trong hệ điều hành cụ thể;

		. Tham số thứ hai chỉ định open mode được sử dụng cho stream. Nó được mô tả bằng một chuỗi các ký tự và mỗi ký tự đều có ý nghĩa đặc biệt của riêng nó (chi tiết hơn sẽ được giải thích sau);

		. Việc mở file phải là very first operation được thực hiện trên stream (có ba trường hợp ngoại lệ cho quy tắc này – chi tiết sẽ được giải thích sau).

	- Open modes: r => read
		. file tương ứng với stream phải tồn tại và có thể đọc được, nếu không, hàm fopen sẽ thất bại.

	- Open modes: w => write
		. file tương ứng với stream không cần phải tồn tại
			> nếu nó không tồn tại, file sẽ được tạo mới
			> nếu file đã tồn tại, nó sẽ bị cắt bớt đến độ dài bằng không (xóa nội dung)
			> nếu việc tạo không thành công (ví dụ, do system permissions) thì hàm fopen sẽ thất bại.

	- Open modes: a => append
		. file tương ứng với stream không cần tồn tại
			> nếu không tồn tại, file sẽ được tạo mới
			> nếu file đã tồn tại, đầu ghi ảo sẽ được đặt vào cuối file (nội dung trước đó của file vẫn được giữ nguyên).

	- Open modes: r+ => read and update
		. file tương ứng với stream phải tồn tại và có thể ghi được, nếu không, hàm fopen sẽ thất bại;
		. Đều đọc và viết được

	- Open modes: w+ => write and update
		. stream sẽ được mở ở chế độ "ghi và cập nhật";
		. file tương ứng với stream không cần phải tồn tại
			> nếu không tồn tại, file sẽ được tạo mới; nội dung trước đó của file sẽ bị loại bỏ (file bị cắt bớt đến độ dài bằng không);
		. Đều đọc và viết được
...
7.1.5.2
	...
	+ Một vài constants hữu ích để detect stream errors:
		...
		- EEXIST: File exists - Xảy ra khi đổi tên một file với tên trước đó của nó.
		...
		. 3 tham số
			...
			. maxsize: số lượng kí tự tối đa có thể stored trong "str";
				...
				Ví dụ: nếu stream read là "No"
					...
					maxsize = 1 = '\0',
					maxsize = 2 = 'N' '\0',
					maxsize >= 3 = 'N' 'o' '\0';		// Chú ý hay sai cái này
...
7.1.7.2
	...
	- Chtrinh sẽ interpret đối số của nó theo the same way:
		...
		Đối số 2 là tên của target (file đích)
...
7.1.7.7
	...
	- Điều đó ngụ ý rằng invocation sau:
...
7.1.8.2
	...
	- Hàm fseek cho phép thiết lập vị trí hiện tại của file. Nếu việc thiết lập thành công, thao tác read/write tiếp theo sẽ ảnh hưởng đến vị trí đó.

	- Vì lý do này, cách thức này trong việc điều khiển file thường được gọi là random access (truy cập ngẫu nhiên), ngược với sequential access (truy cập tuần tự), trong đó dữ liệu được written/read theo thứ tự liên tục.
...
7.1.8.6 LAB
	...
	- Viết một chương trình sao chép chính nó và thêm số thứ tự cho mỗi dòng. Bạn có thể (tạm thời, để đơn giản hóa công việc của bạn) giả định tên file là cố định.

	- Đầu tiên, mở file trong chế độ đọc, sau đó mở file sao chép trong chế độ ghi. Tiếp theo, từng dòng, lấy các dòng từ file nguồn, thêm số thứ tự dòng vào file đích và thêm nội dung của dòng.

	- Nhớ đóng cả hai file.

	- Note: trong một chương trình thực tế, quan trọng để kiểm tra xem tất cả các file cần thiết có tồn tại và bạn có thể tạo một bản sao. Trong chương trình của chúng ta, bạn ít nhất nên kiểm tra xem cả hai file đã được mở thành công hay không.

	- Nếu một trong các hàm fopen thất bại, sau đó in ra thông báo: Error opening a file. và kết thúc chương trình.

		#include <stdio.h>

		int main()
		{
		    FILE    *srfile = fopen("text.txt", "r"),
		            *cpfile = fopen("text2.txt", "w");
		    ...
		    if (cpfile == NULL)
		    {
		        printf("Can't open text2.txt");
		        return 1;
		    }

		    int linenumber = 1;

		    fprintf(cpfile, "%d ", linenumber);
		    
		    char c;
		    while ((c = fgetc(srfile)) != EOF)
		    {
		        fputc(c, cpfile);
		        if (c == '\n')  
		        {
...
7.1.8.7 LAB
	...
	- Viết một chương trình để đếm các chỉ số của một file:

		. Số lượng tất cả các ký tự
		. Số lượng khoảng trắng
		. Số lượng dòng
		. Số lượng tất cả các ký tự viết thường

	- Bạn có thể (tạm thời, để đơn giản hóa công việc của bạn) giả định tên file là cố định. Để tăng tốc độ lập trình, bạn có thể thử nghiệm chương trình của mình trên mã nguồn của nó. Đầu tiên, mở một file ở chế độ đọc. Sau đó, từng ký tự, đọc file và đếm số lượng đã yêu cầu. Nhớ đóng file.

	- Note: trong một chương trình thực tế, quan trọng để kiểm tra xem tất cả các file cần thiết có thực sự tồn tại và bạn có thể đọc nội dung của chúng. Trong chương trình của chúng ta, bạn ít nhất nên kiểm tra xem các file đã được mở thành công hay không.

	- Nếu hàm fopen thất bại, sau đó in ra thông báo: Error opening file.. và kết thúc chương trình. Chương trình chỉ có một lần xuất hiện của đầu ra - kết quả đầu ra có thể thay đổi, tùy thuộc vào nội dung của file.
		...
		int main()
		{
		    ...
		    int numAllchar = 0, numWhitesp = 0, numLines = 1, numAlllowercase = 0;
		    char c;
		    ...
		    while ((c = fgetc(srfile)) != EOF)
		    {
		        ...
		        if (97 <= c && c <= 122)   numAlllowercase++;
		    }

		    fclose(srfile);

		    printf("%d %d %d %d", numAllchar, numWhitesp, numLines, numAlllowercase);
...
7.1.8.8 LAB
	...
	- Viết một chương trình sẽ sao chép một file. Tuy nhiên, nếu đã có một file đích, chương trình sẽ cố gắng tạo một file mới (với một tên file mới) cho đến khi một file đích đã được tạo ra.

	- Nhớ đóng cả hai file.

	- Note: trong một chương trình thực tế, quan trọng để kiểm tra xem tất cả các file cần thiết có thực sự tồn tại và bạn có thể tạo một bản sao. Trong chương trình của chúng ta, bạn ít nhất nên kiểm tra xem cả hai file đã được mở thành công hay không.

	- Nếu hàm fopen cho file nguồn thất bại, sau đó chương trình nên in ra thông báo: Error opening file. và kết thúc chương trình. Nếu quá trình tạo file thất bại, sau đó bạn nên thay đổi tên của file đích.

	- Bạn có thể gắn thêm một số nguyên vào tên file cũ.

	- Hãy nghĩ đến một giới hạn về số lần cố gắng tạo file (nó phụ thuộc vào hệ điều hành của bạn và, tất nhiên, vào nhu cầu cụ thể của chương trình của bạn). Nếu bạn tạo quá nhiều file, có thể khó để sử dụng thư mục chứa những file này.

	Nếu chương trình của bạn đã đạt đến giới hạn này, sau đó nó nên in ra thông báo: Đã đạt đến giới hạn số lượng file. và kết thúc chương trình.
		...
		int main()
		{
		    ...
		    {
		        if (index == 1)
		            strcpy(targetfileName, "target.txt");
		        else
		            sprintf(targetfileName, "target%d.txt", index);
		    ...
		    if (index > 5)
		    {
		        printf("can't create file due to limitation.");
		        return 1;
		    }
...
8.1.2.1 Preprocessor: the #include directive
	- "#include" directive hoạt động như sau:
		. Bộ tiền xử lý tìm file tên filename, nhưng nếu để tên đó trong quotes, thì nó sẽ tìm file đó trong cùng thư mục với file chứa directive
		...
		. File được included cũng có thể chứa chỉ thị #include; thêm nữa, 1 compiler cụ thể có thể impose (áp đặt) 1 giới hạn nhất định số lượng lần lồng nhau của các file
			Kiểu như có 1 file A bao gồm file B, file B bao gồm file C và tiếp tục như vậy. Tuy nhiên, 1 số compiler có thể giới hạn sâu độ lồng nhau này để ngăn chặn việc sử dụng quá nhiều file được nhúng lồng nhau, tránh gây ra vấn đề về quản lý bộ nhớ hoặc thời gian biên dịch không cần thiết.
